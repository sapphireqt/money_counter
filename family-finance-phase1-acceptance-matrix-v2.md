...
| Table | Transfer | Двухстрочная структура | P/table-transfer-1440x900.png | B/table-transfer-1440x900.png | table-transfer: пара «A → B» в колонке счёта; видимость строк — только от валют (USD→EUR: main EUR + списание USD + «→ EUR», 52px; EUR→EUR и EUR→USD при display EUR: только main, 44px); unit «transfer amount visibility depends on currencies only» | PASS |

...
- **Attention flags**: `transactions.flagged` (+ free-text `notes` as the explanation) — the «⚑ Требует внимания» checkbox in the edit form. Purely visual: amber ⚑ in the marker column with the note as a fast tooltip, «⚑» filter toggle, `GET /api/transactions?flagged=1`. Used to mark import oddities for later review.

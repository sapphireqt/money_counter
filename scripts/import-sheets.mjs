...
//   ...optional: --chunk 1000
//   ...--home-thb: THB-era tabs (until ~2025-04) where the "EUR" column is THB
//   ...--cash-thb: with --home-thb, exact «Снятие наличных» from a THB account
//      also credits CASH THB with the same baht amount (2024 tabs; NOT for
//      2025-01..03 where withdrawn baht became CASH EUR via manual legs)
//
...
function buildRows(records, report, layout, opts = {}) {
  ...
  for (const rec of records) {
    ...
    if (opts.homeThb && (label === "наличные" || label === "cash")) {
      ...
      // --cash-thb (deep-Thailand tabs): ALL cash is baht, incl. rows whose
      // home-equivalent cell lost its formula in the CSV export.
      if (opts.cashThbWithdrawals || (Number.isFinite(home) && Math.abs(amount - home) < 0.01)) {
    ...
    if (isExpense && exactCash && acct.name !== "CASH EUR" && acct.name !== "CASH THB") {
      if (opts.homeThb && acct.currency !== "EUR") {
        if (opts.cashThbWithdrawals && acct.currency === "THB") {
          // Deep-Thailand era (2024 tabs): baht withdrawn from a THB bank stays
          // baht — credit CASH THB with the SAME amount. Opt-in via --cash-thb
          // because the CSV alone can't tell this apart from the leaving-Thailand
          // months (2025-01..03) where withdrawn baht was exchanged into the
          // CASH EUR wallet and the credit leg was entered manually.
          out.push({ accountName: "CASH THB", currency: "THB", date, amount, direction: "income", description, category: "" });
          report.cashWithdrawalsThb += 1;
        } else {
...
for (let i = 0; i < argv.length; i += 1) {
  ...
  else if (argv[i] === "--cash-thb") { cashThbWithdrawals = true; }
...
const report = {
  ...
  cashThb: 0, cashCrossThb: [], cashWithdrawalsThb: 0,
};
let rows = [];
for (const file of files) {
  ...
  const built = buildRows(recs, report, layout, { homeThb, cashThbWithdrawals });
  console.log(`• ${file}: ${recs.length} строк CSV → ${built.length} операций [раскладка ${layout.name}${homeThb ? ", дом. валюта THB" : ""}${cashThbWithdrawals ? ", снятия → CASH THB" : ""}]`);
...
if (report.cashWithdrawalsThb) console.log(`Снятий наличных → приход на CASH THB (та же сумма в батах): ${report.cashWithdrawalsThb} (связать в приложении: «Найти переводы»)`);

...
// account gets sort_order = its index. Ids not in the list keep 9999 and fall
// back to the alphabetical tail.
export async function POST(request: Request) {
  try {
    ...
    await d1.batch(
      ids.map((id, index) =>
        d1
          .prepare("UPDATE accounts SET sort_order = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?")
          .bind(index, id)
      )
    );
